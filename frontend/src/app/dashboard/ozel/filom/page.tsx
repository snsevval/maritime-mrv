...
import { Star, AlertTriangle, ChevronLeft, ChevronRight, Search, RotateCcw, Download } from "lucide-react";
import { ozelApi, getErrorMessage } from "@/lib/api";
import type { FilomItem, FilomList } from "@/types";
...
const MP_DURUMLAR = [
  ...
  { deger: "archived", etiket: "Arşivlendi" },
];
...
function durumuCevir(durum?: string | null, tip: "mp" | "rapor" | "vr" = "rapor") {
  if (!durum) return null;
  const harita: Record<string, { etiket: string; renk: string }> = {
    active:             { etiket: "Aktif",       renk: "bg-green-100 text-green-700" },
    draft:              { etiket: "Taslak",      renk: "bg-gray-100 text-gray-600" },
    archived:           { etiket: "Arşivlendi",  renk: "bg-gray-100 text-gray-500" },
    submitted:          { etiket: "Gönderildi",  renk: "bg-blue-100 text-blue-700" },
    under_verification: { etiket: "Doğrulamada", renk: "bg-yellow-100 text-yellow-700" },
    approved:           { etiket: "Onaylandı",   renk: "bg-green-100 text-green-700" },
    rejected:           { etiket: "Reddedildi",  renk: "bg-red-100 text-red-700" },
    pending:            { etiket: "Bekliyor",    renk: "bg-yellow-100 text-yellow-700" },
  };
  return harita[durum] ?? { etiket: durum, renk: "bg-gray-100 text-gray-600" };
}
...
export default function FilomPage() {
  ...
  const [sayfaBasi, setSayfaBasi] = useState(10);
  const [data, setData] = useState<FilomList>(BOŞ_VERİ);
  ...
  useEffect(() => {
    try {
      const kayit = localStorage.getItem("filom_favoriler");
      if (kayit) setFavoriler(new Set(JSON.parse(kayit)));
    } catch { /* ignore */ }
  }, []);
  ...
  const yukle = useCallback(async (f: typeof BOŞ_FİLTRE, s: number, sp: number) => {
    ...
    try {
      ...
      if (f.ship_type)       params.ship_type = f.ship_type;
      if (f.flag)            params.flag = f.flag;
      if (f.company)         params.company = f.company;
      if (f.mp_status)       params.mp_status = f.mp_status;
      if (f.report_status)   params.report_status = f.report_status;
      if (f.vr_status)       params.vr_status = f.vr_status;
      if (f.gt_min)          params.gt_min = parseInt(f.gt_min);
      if (f.gt_max)          params.gt_max = parseInt(f.gt_max);
      if (f.reporting_period) params.reporting_period = parseInt(f.reporting_period);
      const res = await ozelApi.filom(params);
      setData(res.data as FilomList);
    } catch (e) {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="w-8 px-3 py-2.5 text-center text-xs font-semibold text-gray-500 uppercase tracking-wide">Uyarı</th>
                <th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">İsim</th>
                <th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Bayrak</th>
                <th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Gemi Tipi</th>
                <th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">MP Durumu</th>
            ...
            <tbody>
              {yukleniyor ? (
                ...
                data.items.map((gemi: FilomItem) => {
                  const mpD = durumuCevir(gemi.mp_status, "mp");
                  const rD = durumuCevir(gemi.report_status, "rapor");
                  const vrD = durumuCevir(gemi.vr_status, "vr");
                  return (
                    <tr key={gemi.id} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="px-3 py-2.5 text-center">
                        <button onClick={() => favoriToggle(gemi.id)}>
                          <Star className={cn("w-4 h-4", favoriler.has(gemi.id) ? "fill-yellow-400 text-yellow-400" : "text-gray-300")} />
                        </button>
                      </td>
                      <td className="px-3 py-2.5 text-center text-gray-300">—</td>
                      <td className="px-3 py-2.5 font-medium text-navy-700">{gemi.name}</td>
                      <td className="px-3 py-2.5 text-gray-600">{gemi.flag}</td>
                      <td className="px-3 py-2.5 text-gray-600">{gemi.ship_type || "—"}</td>
                      <td className="px-3 py-2.5">
                        {mpD ? <span className={cn("badge", mpD.renk)}>{mpD.etiket}</span> : <span className="text-gray-300">—</span>}
                      </td>
                      <td className="px-3 py-2.5">
                        {rD ? <span className={cn("badge", rD.renk)}>{rD.etiket}</span> : <span className="text-gray-300">—</span>}
                      </td>
                      <td className="px-3 py-2.5">
                        {vrD ? <span className={cn("badge", vrD.renk)}>{vrD.etiket}</span> : <span className="text-gray-300">—</span>}
                      </td>
                      <td className="px-3 py-2.5 text-gray-600">{gemi.reporting_period ?? "—"}</td>
                      <td className="px-3 py-2.5 text-gray-600">{gemi.verifier_name || "—"}</td>
                      <td className="px-3 py-2.5 text-gray-600">{gemi.company_name || "—"}</td>
                    </tr>
                  );
                })
              )}
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-400">
              {data.total > 0 ? `${(sayfa - 1) * sayfaBasi + 1} - ${Math.min(sayfa * sayfaBasi, data.total)} / ${data.total} arası gösteriliyor` : "Gösterilecek veri yok."}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="btn-primary flex items-center gap-1.5 text-xs h-7 px-3">
              <Download className="w-3 h-3" /> Toplu veri indirme
            </button>
          </div>
        </div>
      </div>
